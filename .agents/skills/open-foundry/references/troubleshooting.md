...
| Invoke returns "configuration: no foundry provider" or model 401 | `OF_OPENAI_BASE_URL`/`OF_OPENAI_MODEL` not set in azd env, or the model key/managed-identity auth is wrong. Reconfigure via `configure-env.mjs`. |
